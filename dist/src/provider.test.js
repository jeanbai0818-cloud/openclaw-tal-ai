...
const withKey = { resolveProviderApiKey: () => ({ apiKey: "300004631:test" }) };
const withoutKey = { resolveProviderApiKey: () => ({ apiKey: undefined }) };
describe("mlops-claude", () => {
    ...
    it("includes expected Claude models", async () => {
        const result = await mlopsClaudeProvider.catalog.run(withKey);
        const ids = result?.providers?.["mlops-claude"]?.models?.map((m) => m.id);
    ...
    it("staticCatalog returns models without auth", async () => {
        const result = await mlopsClaudeProvider.staticCatalog.run({});
        expect(result?.providers?.["mlops-claude"]?.api).toBe("anthropic-messages");
        expect(result?.providers?.["mlops-claude"]?.baseUrl).toBe("https://ai-service.tal.com");
        const ids = result?.providers?.["mlops-claude"]?.models?.map((m) => m.id);
        expect(ids).toContain("claude-sonnet-4.6");
...
describe("tal-mlops", () => {
    ...
    it("includes MiniMax and GLM models", async () => {
        const result = await talMlopsProvider.catalog.run(withKey);
        const ids = result?.providers?.["tal-mlops"]?.models?.map((m) => m.id);
    ...
    it("staticCatalog returns models without auth", async () => {
        const result = await talMlopsProvider.staticCatalog.run({});
        expect(result?.providers?.["tal-mlops"]?.api).toBe("openai-completions");
        expect(result?.providers?.["tal-mlops"]?.baseUrl).toBe("https://ai-service.tal.com/openai-compatible/v1");
        const ids = result?.providers?.["tal-mlops"]?.models?.map((m) => m.id);
        expect(ids).toContain("MiniMax-M2.7");
...
describe("claw", () => {
    ...
    it("claude models have per-model api override to anthropic-messages", async () => {
        const result = await clawProvider.catalog.run(withKey);
        const models = result?.providers?.["claw"]?.models ?? [];
        const claudeModels = models.filter((m) => m.id.startsWith("claude-") || m.id.startsWith("glm-"));
    ...
    it("openai-compat models have no api override (inherit provider default)", async () => {
        const result = await clawProvider.catalog.run(withKey);
        const models = result?.providers?.["claw"]?.models ?? [];
        const oaiModels = models.filter((m) => ["gpt-5.3-codex", "gpt-5.2-codex", "kimi-k2.5", "gemini-3-flash",
    ...
    it("gemini-3-flash has requiresToolResultName compat flag", async () => {
        const result = await clawProvider.catalog.run(withKey);
        const gemini = result?.providers?.["claw"]?.models?.find((m) => m.id === "gemini-3-flash");
    ...
    it("staticCatalog returns models without auth", async () => {
        const result = await clawProvider.staticCatalog.run({});
        expect(result?.providers?.["claw"]?.api).toBe("openai-completions");
        expect(result?.providers?.["claw"]?.baseUrl).toBe("https://ai-service.tal.com/claw/v1");
        expect(result?.providers?.["claw"]?.headers?.["X-Agent-Channel"]).toBe("jcfwzt-sre-openclaw");
        expect(result?.providers?.["claw"]?.models).toHaveLength(16);
        const ids = result?.providers?.["claw"]?.models?.map((m) => m.id);
        expect(ids).toContain("claude-sonnet-4.6");
